...
public final class ProgressBarTab {
    ...
    static void build(NightNotifierConfigScreen screen, int left, int right, int yLeft, int yRight, int w, int h) {
        ...
        screen.addDrawableChild(ButtonWidget.builder(net.minecraft.text.Text.literal("P"), b -> {
            MinecraftClient.getInstance().setScreen(ColorPickerScreen.open(screen, color0Field.getText(), picked -> color0Field.setText(picked)));
        }).dimensions(right + w + 4, yRight, 22, 20).build());
        yRight += 24;
        TextFieldWidget color1Field = screen.strField(right, yRight, w, cfg.progressSectionColor1, "Mid Night Color (hex)", () -> {});
        screen.addDrawableChild(ButtonWidget.builder(net.minecraft.text.Text.literal("P"), b -> {
            MinecraftClient.getInstance().setScreen(ColorPickerScreen.open(screen, color1Field.getText(), picked -> color1Field.setText(picked)));
        }).dimensions(right + w + 4, yRight, 22, 20).build());
        yRight += 24;
        TextFieldWidget color2Field = screen.strField(right, yRight, w, cfg.progressSectionColor2, "Night Ending Color (hex)", () -> {});
        screen.addDrawableChild(ButtonWidget.builder(net.minecraft.text.Text.literal("P"), b -> {
            MinecraftClient.getInstance().setScreen(ColorPickerScreen.open(screen, color2Field.getText(), picked -> color2Field.setText(picked)));
        }).dimensions(right + w + 4, yRight, 22, 20).build());
        yRight += 24;
        TextFieldWidget color3Field = screen.strField(right, yRight, w, cfg.progressSectionColor3, "Morning Warning Color (hex)", () -> {});
        screen.addDrawableChild(ButtonWidget.builder(net.minecraft.text.Text.literal("P"), b -> {
            MinecraftClient.getInstance().setScreen(ColorPickerScreen.open(screen, color3Field.getText(), picked -> color3Field.setText(picked)));
        }).dimensions(right + w + 4, yRight, 22, 20).build());
        yRight += 24;

        screen.addDrawableChild(ButtonWidget.builder(net.minecraft.text.Text.literal("Apply Progress Bar Settings"), b -> {
